...
impl RagService {
  ...
  pub fn index_add_files<R: Runtime>(
    ...
    for path in file_paths {
      let Some(candidate) = self.prepare_file_candidate(project_id, &path, root_dir.as_deref())?
      else {
  ...
  pub fn index_sync_project<R: Runtime>(
    ...
    let root_dir = if let Some(root_dir) = root_dir_override {
      ...
      let root = get_project_root(app, project_id).ok_or_else(|| "project root not set".to_string())?;
      if !root.exists() {
  ...
  fn build_chunks(
    ...
    for (index, (chunk, embedding)) in chunks.into_iter().zip(embeddings.into_iter()).enumerate() {
...
mod tests {
  ...
  impl RagStore for SharedStore {
    ...
    fn search(
      ...
      let guard = self.inner.lock().map_err(|_| "store poisoned".to_string())?;
  ...
  fn index_add_and_search() {
    ...
    let shared = SharedStore { inner: store.clone() };
  ...
  fn sync_removes_deleted_file() {
    ...
    assert_eq!(
      guard.chunk_count_for_file("proj_sync", &file2_id),
      0
    );
