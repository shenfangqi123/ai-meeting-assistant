...
fn chunks_to_batch(chunks: &[ChunkRecord], dimension: usize) -> Result<RecordBatch, String> {
    ...
    let chunk_indexes = Int32Array::from(chunks.iter().map(|c| c.chunk_index).collect::<Vec<_>>());
    let texts = StringArray::from(chunks.iter().map(|c| c.text.as_str()).collect::<Vec<_>>());
    let updated_at = StringArray::from(
        chunks
            .iter()
...
fn files_to_batch(records: &[FileRecord]) -> Result<RecordBatch, String> {
    ...
    let is_deleted = BooleanArray::from(records.iter().map(|c| c.is_deleted).collect::<Vec<_>>());
    let updated_at = StringArray::from(
        records
            .iter()
            .map(|c| c.updated_at.as_str())
