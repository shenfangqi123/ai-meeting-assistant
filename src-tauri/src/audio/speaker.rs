...
impl SpeakerDiarizer {
  ...
  pub fn process_window(
    ...
    if let Ok(switches) = self.embedder.detect_switches(window, &self.config.switch_params) {
...
impl SpeakerEmbedder {
  ...
  fn detect_switches(&mut self, samples: &[f32], params: &SwitchParams) -> Result<Vec<u64>, String> {
    let window_samples = ms_to_samples(params.window_ms, TARGET_SAMPLE_RATE);
    if window_samples == 0 || samples.len() < window_samples {
      return Ok(Vec::new());
    }
    let hop_samples = ms_to_samples(params.hop_ms, TARGET_SAMPLE_RATE).max(1).min(window_samples);
    ...
    while start + window_samples <= samples.len() {
      ...
      if let Some(prev) = prev_embedding.as_ref() {
        ...
        if consecutive_hits >= params.consecutive_hits {
          let time_ms =
            ((start + window_samples / 2) as f32 / TARGET_SAMPLE_RATE as f32 * 1000.0)
              .round() as u64;
...
fn resolve_model_path(path: Option<&str>, resource_dir: Option<PathBuf>) -> Option<PathBuf> {
  ...
  candidates.into_iter().find(|path| path.exists()).or(Some(candidate))
}
